...
public:
    ...
    [[nodiscard]]
    bool is_child_guard_public() const final {
        return true;
    }
};
...
TEST_CASE("Entity Manager") {
    ...
    SECTION("add child") {
        ...
        SECTION("remove multiple children") {
            ...
            REQUIRE(dynamic_cast<TestEntity&>(root.get_child_by_id(manager, child1_id)).value == 666);
            REQUIRE(dynamic_cast<TestEntity&>(root.get_child_by_index(manager, 0)).value == 666);
            REQUIRE(dynamic_cast<TestEntity&>(root.get_child_by_id(manager, child3_id)).value == 668);
