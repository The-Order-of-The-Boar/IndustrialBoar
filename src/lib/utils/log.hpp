...
public:
    ...
    [[nodiscard]] char const* what() const noexcept override { return this->message.c_str(); }
};
...
[[noreturn]]
void panic(std::string_view message, std::source_location location = std::source_location::current());
...
[[noreturn]]
void graceful_exit(std::source_location location = std::source_location::current());
