...
EntityID Entity::get_child_by_index(const EntityManager &manager, size_t index) const {

    ib_debug_assert(this->has_child_guard());
    return manager.get_child_id_by_index(this->child_guard.value().get_parent_id(), index);
}

EntityID Entity::get_child_by_name(const EntityManager &manager, std::string_view name) const {
