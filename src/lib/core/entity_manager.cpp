...
EntityID Entity::add_child(EntityManager &manager, std::unique_ptr<Entity> child_entity, std::optional<std::string> name) {
...
void Entity::set_child_name(EntityManager &manager, size_t index, const std::string &name) {

    ib_debug_assert(this->has_child_guard());
    manager.set_child_name(this->child_guard.value(), index, name);
}

std::optional<std::string> Entity::get_child_name_by_index(const EntityManager &manager, size_t index) const {

    ib_debug_assert(this->has_child_guard());
    return manager.get_child_name_by_index(this->child_guard.value().get_parent_id(), index).value();
}
...
EntityID Entity::get_child_id_by_index(const EntityManager &manager, size_t index) const {

    ib_debug_assert(this->has_child_guard());
    return manager.get_child_id_by_index(this->child_guard.value().get_parent_id(), index);
}

EntityID Entity::get_child_id_by_name(const EntityManager &manager, std::string_view name) const {

    ib_debug_assert(this->has_child_guard());
    return manager.get_child_id_by_name(this->child_guard.value().get_parent_id(), name);
}

size_t Entity::get_child_count(const EntityManager &manager) const {
...
EntityManager::EntityManager(std::unique_ptr<Entity> root_entity): root_id(entity_counter++) {
    ...
    this->entities.emplace(std::pair<size_t, EntityData>(this->root_id.id, EntityData{std::move(root_entity), std::nullopt}));
...
EntityID EntityManager::add_child(EntityChildGuard guard, std::unique_ptr<Entity> child_entity) {
    ...
    this->entities.emplace(std::pair<size_t, EntityData>(child_id.id, EntityData{std::move(child_entity), guard.get_parent_id()}));
...
void EntityManager::set_child_name(EntityChildGuard guard, size_t child_index, std::string const& name) {
...
void EntityManager::set_child_index(EntityChildGuard guard, size_t child_index, size_t new_index) {
    ...
    parent.children.insert(parent.children.begin() + new_index, child_id); // NOLINT(*-narrowing-conversions)
...
std::optional<std::string> EntityManager::get_child_name_by_index(EntityID parent_id, size_t child_index) const {
